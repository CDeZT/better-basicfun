...
import { installReasoningGuard } from './reasoning.js'

export const name = 'foundation-enhancements'
export const inject = [
  'workspaceRegistry',
  'skills',
  'tools',
...
export const Config = z.object({
  ...
  reasoningGuard: z.boolean().default(true),
})
...
export async function apply(ctx, config = {}) {
  ...
  if (config.reasoningGuard !== false) installReasoningGuard(ctx)

  ctx.systemPrompt.context({
    name: 'foundation-enhancements:resources',
    order: 115,
  ...
  ctx.systemPrompt.context({
    name: 'foundation-enhancements:memory',
    order: 116,
    text: (request) => isDefaultSession(request, paths.workspacePath)
  ...
  ctx.logger?.info?.(`dsh-foundation-enhancements: ready at ${paths.workspacePath}`)
}
