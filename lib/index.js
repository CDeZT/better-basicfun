...
import { installCliProxyBridge } from './cliproxyapi-bridge.js'

export const name = 'better-basicfun'
export const inject = [
  'workspaceRegistry',
  'skills',
  'tools',
...
export const Config = z.object({
  ...
  cliProxySync: z.boolean().default(true),
  cliProxyProbeAllCustomProviders: z.boolean().default(true),
  cliProxyRoutes: z.array(z.string()).default([]),
  cliProxyRefreshMinutes: z.number().step(1).min(1).max(1440).default(15),
  cliProxyRequestTimeoutMs: z.number().step(1).min(250).max(120_000).default(10_000),
})
...
export async function apply(ctx, config = {}) {
  ...
  const cliProxy = installCliProxyBridge(ctx, config)

  ctx.systemPrompt.context({
    name: 'better-basicfun:resources',
    order: 115,
  ...
  ctx.tools.register(resourceTool(ctx, paths, workspace, cliProxy))
  ctx.logger?.info?.(`better-basicfun: ready at ${paths.workspacePath}`)
}

function resourceTool(ctx, paths, workspace, cliProxy) {
  return {
    name: 'dsh_resources',
    ...
    async execute(args, exec) {
      ...
      const value = await inspectResource(ctx, paths, workspace, cliProxy, args, exec)
...
async function inspectResource(ctx, paths, workspace, cliProxy, args, exec) {
  const resources = resourcePaths(paths)
  ...
  switch (args.kind) {
    ...
    case 'credential':
      return inspectCredential(ctx, args, offset, listLimit)
    case 'cliproxyapi':
      return {
        source: 'better-basicfun host-side CLIProxyAPI catalog synchronizer',
...
async function inspectCredential(ctx, args, offset, listLimit) {
  ...
  return {
    sensitive: true,
    ...pageArray(records, offset, listLimit),
