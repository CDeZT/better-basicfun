...
/* ---- Composer model-menu slider (mounted inside the OFFICIAL menu) ----
   Interaction and visual structure are adapted from HanaAyane's
   dsh-reasoning-effort (MIT). This bundle keeps the white knob, adds clear
   level ticks and a restrained animation, and prefixes all classes with bre-
   so the retired upstream bundle cannot collide with this merged plugin. */
.bre-slider-body {
...
.bre-effort-track::before { content: ""; position: absolute; inset: 0; z-index: 1; pointer-events: none; opacity: .62; background: repeating-linear-gradient(to right, rgba(255,255,255,.72) 0 1px, transparent 1px calc(100% / (var(--bre-level-count) - 1))); }
...
.bre-effort.is-dragging .bre-effort-knob {
  ...
  box-shadow:
    ...
    0 0 16px rgba(74,145,255,.58),
    0 0 23px rgba(171,53,255,.36),
...
body:not([data-ds-dark-theme]) .bre-effort-track::before {
  background:
    linear-gradient(90deg, #fff 0%, #e2f0ff 20%, #a8d0fb 57%, #438fdf 100%),
    repeating-linear-gradient(to right, transparent 0 1px, rgba(48,104,170,.34) 1px 2px, transparent 2px calc(100% / (var(--bre-level-count) - 1)));
  background-size: var(--bre-progress) 100%, 100% 100%;
...
@keyframes bre-effort-dark-breathe {
  ...
  50% { box-shadow: inset 0 1px 0 rgba(220,214,255,.24), 0 0 14px rgba(111,66,255,.25); }
}
@keyframes bre-effort-light-breathe {
  ...
  50% { box-shadow: inset 0 1px 0 rgba(255,255,255,.96), inset 0 0 0 1px rgba(31,102,190,.22), 0 0 13px rgba(31,105,201,.14); }
}
.bre-slider-hint {
...
.bre-advanced-row { display: flex; align-items: center; justify-content: space-between; min-height: 38px; padding: 0 14px; width: 100%; border: 0; color: inherit; background: transparent; font: inherit; font-size: 12px; text-align: left; cursor: pointer; }
.bre-advanced-row::after { content: "›"; font-size: 18px; line-height: 1; opacity: .45; }
.bre-advanced-row:hover { background: var(--dsw-alias-fill-tertiary, rgba(120,125,140,.09)); }
.bre-model-row {
  display: grid;
